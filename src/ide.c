...
// this implementation is adapted from https://github.com/xing1357/SimpleOS/blob/main/src/ide.c
// added inits and boudns check on read/write, cmd selection now uses LUT
...
static uint8 ide_read_register(uint8 channel, uint8 reg) {
    if (channel >= MAXIMUM_CHANNELS) return 0xFF;
...
static void ide_write_register(uint8 channel, uint8 reg, uint8 data) {
    if (channel >= MAXIMUM_CHANNELS) return;
...
uint8 ide_ata_access(uint8 direction, uint8 drive, uint32 lba, uint8 num_sectors, void* buffer) {
    ...
    // lbamode: 0=CHS, 1=LBA28, 2=LBA48
    // direct: 0=read, 1=write
    static const uint8 pio_cmd[3][2] = {
        { ATA_CMD_READ_PIO, ATA_CMD_WRITE_PIO },
        { ATA_CMD_READ_PIO, ATA_CMD_WRITE_PIO },
        { ATA_CMD_READ_PIO_EXT, ATA_CMD_WRITE_PIO_EXT },
    };
    static const uint8 dma_cmd[3][2] = {
    ...
    if (lba_mode > LBA_MODE_48 || (direction != ATA_READ && direction != ATA_WRITE)) {
        return 5;
    }
    cmd = dma ? dma_cmd[lba_mode][direction] : pio_cmd[lba_mode][direction];
    ...
    if (dma) {
        ...
        for (i = 0; i < num_sectors; i++) {
            ...
            // save ds segment and repeat outsw(write stream of shorts) instruction util no of sectors are written to ide device
            asm("pushw %ds");
...
// start from lba = 0
int ide_read_sectors(uint8 drive, uint8 num_sectors, uint32 lba, void* buffer) {
...
// start from lba = 0
int ide_write_sectors(uint8 drive, uint8 num_sectors, uint32 lba, void* buffer) {
    // 1: Check if the drive presents:
    if (drive >= MAXIMUM_IDE_DEVICES || g_ide_devices[drive].reserved == 0) {
        printf("IDE ERROR: Drive not found\n");
        return -1;
