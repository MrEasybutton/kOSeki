...
// adpated from https://github.com/xing1357/SimpleOS/blob/main/src/console.c
...
void console_clear(VGA_COLOR_TYPE fore_color, VGA_COLOR_TYPE back_color) {
    ...
    cursor_pos_x = 0;
    cursor_pos_y = 0;
    set_cur_pos(cursor_pos_x, cursor_pos_y);
}
...
void ltoa(char *buf, unsigned long long n) {
    char *p = buf;
    unsigned long long shifter = n;
    do {
        p++;
        shifter /= 10;
    } while (shifter);
    ...
    while ((c = *format++) != 0) {
        if (c != '%')
            ...
            switch (c) {
                ...
                case 'x':
                    if (l == 2 && c == 'u') {
                        ltoa(buf, *((unsigned long long *)arg));
                        arg += 2;
                    } else {
                        itoa(buf, c, *((int *)arg++));
                    }
