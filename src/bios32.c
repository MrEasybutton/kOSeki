...
// adapted from https://github.com/xing1357/SimpleOS/blob/main/src/bios32.c
// did null checks on service call, exec_bios32_function global is removed for clarity

extern GDT g_gdt[NO_GDT_DESCRIPTORS];

//might as well (roku nana)
#define BIOS32_CODE_SEG 6
#define BIOS32_DATA_SEG 7

// will be copied when call service
void bios32_init() {
    // 16bit code segment
    gdt_set_entry(BIOS32_CODE_SEG, 0, 0xffffffff, 0x9A, 0x0f);
    // 16bit data segment
    gdt_set_entry(BIOS32_DATA_SEG, 0, 0xffffffff, 0x92, 0x0f);
    // real mode gdt ptr
...
void bios32_service(uint8 interrupt, REGISTERS16 *in, REGISTERS16 *out) {
    if (!in || !out) return;
    if (BIOS32_END <= BIOS32_START) {
        printf("[BIOS32] invalid code range\n");
        return;
    }
    ...
    void *in_reg16_address = REBASE_ADDRESS(&bios32_out_reg16_ptr);
